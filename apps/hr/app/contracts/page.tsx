...
import { getAllContracts, getExpiringContracts, type ContractWithSign } from '@/actions/contract-actions'
...
export default async function ContractsPage() {
  ...
  const contracts: ContractWithSign[] = result.success ? result.data : []
  ...
  const signCompleted  = contracts.filter(c => c.status === 'completed').length
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: '전자서명 완료', value: signCompleted, unit: '건', color: 'text-green-700' },
        ].map(card => (
