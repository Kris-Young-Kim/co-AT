...
function AdminSignInForm() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      // 타임아웃 설정 (15초 - GET·POST·프로필생성까지 전체 흐름 커버)
      const timeoutId = setTimeout(() => {
        // #region agent log
        fetch('http://127.0.0.1:7243/ingest/019d36bc-4964-4ab1-b760-13c472a4ead0',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'admin/page.tsx:timeout',message:'15초 타임아웃 발생',data:{},hypothesisId:'H1',timestamp:Date.now()})}).catch(()=>{});
        // #endregion
      ...
      fetch('/api/admin/session', {
        ...
        .catch((error) => {
          clearTimeout(timeoutId)
          // #region agent log
          fetch('http://127.0.0.1:7243/ingest/019d36bc-4964-4ab1-b760-13c472a4ead0',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'admin/page.tsx:catch',message:'fetch catch 달림',data:{error:String(error)},hypothesisId:'H5',timestamp:Date.now()})}).catch(()=>{});
          // #endregion
