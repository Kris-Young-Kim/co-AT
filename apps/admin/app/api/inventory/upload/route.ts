  ...
  try {
    ...
    if (uploadError) {
      ...
      if (
        ...
        return NextResponse.json(
          { 
            error: "Storage ë²„í‚·???¤ì •?˜ì? ?Šì•˜?µë‹ˆ?? Supabase ?€?œë³´?œì—??'inventory' ë²„í‚·???ì„±?´ì£¼?¸ìš”.",
    ...
    console.error("[?¬ê³  ?´ë?ì§€ ?…ë¡œ?? ?ˆìƒì¹?ëª»í•œ ?¤ë¥˜:", error)
