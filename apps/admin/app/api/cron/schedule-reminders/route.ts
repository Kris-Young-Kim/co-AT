...
export async function GET(request: Request) {
  try {
    ...
    console.log(`[Schedule Reminders] 내일 일정 조회: ${tomorrowStr}`)
    ...
    if (!schedules || schedules.length === 0) {
      console.log("[Schedule Reminders] 내일 일정 없음")
      return NextResponse.json({
        success: true,
    ...
    console.log(`[Schedule Reminders] ${schedules.length}건 일정 발견`)
    ...
    console.log(`[Schedule Reminders] 완료: 총 ${totalNotifications}건 알림 생성`)
