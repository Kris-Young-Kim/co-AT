...
한국어로 답변하세요.`,
  ...
  document: `당신은 강원도 보조기기센터의 문서 생성 AI입니다.
SOAP 노트, 업무 보고서 등 각종 문서 작성을 지원합니다.
정확하고 전문적인 어조로 문서를 작성하세요.
한국어로 작성하세요.`,
...
export async function runOrchestratorStream(
  ...
  try {
    // ── STEP 1: 라우팅 결정 ─────────────────────────────────────────────────
    const routingModel = getGeminiModel("gemini-2.5-flash")
    ...
    const agentModel = getGeminiModel("gemini-2.5-flash")
