...
export function LifecycleChangedEmail({ clientName, newStatus, staffName }: Props) {
  return (
    <Html lang="ko">
      <Head />
      <Body style={{ fontFamily: 'sans-serif', backgroundColor: '#f9fafb' }}>
        <Container style={{ maxWidth: 560, margin: '40px auto', background: '#fff', borderRadius: 8, padding: 32 }}>
          <Heading style={{ fontSize: 20, color: '#111' }}>대상자 생애주기 변경 안내</Heading>
          <Hr />
          {staffName && <Text>담당자 <strong>{staffName}</strong>님께 알립니다.</Text>}
          <Text><strong>{clientName}</strong>님의 생애주기 상태가 변경되었습니다.</Text>
...
export const lifecycleChangedSubject = (clientName: string) =>
  `[GWATC] ${clientName}님 생애주기 상태 변경`
