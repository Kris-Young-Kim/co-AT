...
> `apps/web/app/(portal)/apps/page.tsx` — ADMIN → 전체 7개 앱, STAFF → `apps[]` 목록만, 일반 사용자 → `/mypage` 리다이렉트
> 포털 헤더 + 모바일 하단 내비게이션에 "앱" 버튼 조건부 표시 (`showApps` prop)

| 항목 | 상태 |
|------|------|
| 진입점 `/apps` (ADMIN/STAFF 전용) | ✅ |
| 앱 카드 — 권한 기반 필터링 | ✅ |
| ADMIN → 전체 앱 / STAFF → apps[] 목록 | ✅ |
| 모바일 하단 내비 "앱" 버튼 | ✅ |
| 비직원 → /mypage 리다이렉트 | ✅ |
...
| 사이드바 섹션 재편 (콘텐츠·업무·시스템) + 배너 관리 웹 관리 탭 통합 | ✅ |
...
| 사이드바 섹션 재편 (대상자·상담·서비스·교부사업·지식·관리) | ✅ |
...
| 사이드바 섹션 재편 (기기 관리·제작·사후관리·현황·분석) | ✅ |
| **홈 대시보드 고도화** — 연체·반납임박 목록 패널, 인라인 반납/연장 액션 | ✅ |
| **연체 관리** — 대여 목록 연체 뷰: 연락처 컬럼 + 인라인 반납/연장 버튼 | ✅ |

### 대상자-자산 바코드 매칭 시스템

> **목표**: 대상자 바코드 + 보조기기 바코드를 스캔해서 대여/배정을 빠르게 연결
> **범위**: eval 앱(대상자 바코드 관리) ↔ inventory 앱(기기 바코드) 교차 연동

#### DB

| 항목 | 상태 |
|------|------|
| `clients` 테이블에 `qr_token` UUID 컬럼 추가 (migration 094) | ✅ |
| `inventory` 테이블 `barcode`/`qr_token` 컬럼 — 이미 존재 (migration 031/047) | ✅ |

#### eval 앱 — 대상자 QR 코드

| 기능 | 상태 |
|------|------|
| 대상자 상세 페이지 — QR 코드 자동 표시 (`ClientQrLabel` 컴포넌트) | ✅ |
| 개별 라벨 인쇄 (`/print/client-label/[clientId]`) | ✅ |
| 일괄 라벨 인쇄 A4 (`/print/client-labels`) — 다중 선택 + 3열 레이아웃 | ✅ |
| eval 사이드바 "QR 라벨 인쇄" 메뉴 추가 | ✅ |
| 바코드 스캔으로 대상자 검색 (`/clients?qr=...`) | ✅ |

#### inventory 앱 — 자산-대상자 매칭

| 기능 | 상태 |
|------|------|
| `/scan/match` — 2단계 스캔 매칭 페이지 (대상자 QR → 기기 QR → 대여 생성) | ✅ |
| inventory 사이드바 "스캔 대여" 메뉴 추가 | ✅ |
| 반납 스캔 — 기기 QR 스캔 → 해당 대여 자동 종료 | ✅ |
| 대여 상세에서 대상자 정보 (eval 앱 링크) 표시 | ✅ |
...
#### ✅ Phase H-2 — E-5 중앙보조기기센터 보고 양식 자동 집계 (완료 2026-06-28)
> 서비스 기록 확정 → 월별 보고 양식 자동 집계, 중복 수작업 제거
- [x] **eval** — 서비스 기록 확정 시 보고 양식 항목 자동 집계 (월별)
- [x] **eval/stats** — 보고서 화면 연동 (현황 요약 + 엑셀 내보내기)

#### ✅ Phase H-4 — E-3 지식관리 마무리 + inventory scan 안정화 (완료 2026-06-28)
> 담당자 인수인계 자동화, 품목별 지식베이스(migration 106), scan QR 에러 처리 개선
- [x] **eval** — ClientStaffPanel (담당자 표시/변경 UI) + changeAssignedStaff 서버 액션
- [x] **eval** — StaffHandoverEmail 템플릿 + notifyStaffHandover (C-9) Resend 발송
- [x] **eval** — eval_product_knowledge 테이블 (migration 106) + CRUD 액션
- [x] **eval** — ProductKnowledgePanel + /knowledge 탭 2개 구조 개편
...
- [x] **eval** — Phase G: 의뢰처 CRM (/referrers 4탭) + 담당자 비활성화 + 협력활동 이력 + 분기별 의뢰 현황
- [x] **eval** — Phase G: 대상자 세그먼트 (/segments) 저장 가능 필터 (장애유형·시군·서비스유형·생애주기)
- [x] **eval** — Phase G: 보호자 다중 연락처 (ClientGuardiansPanel) + 주 보호자 지정 + 비활성화
