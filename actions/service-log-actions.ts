...
export async function createServiceLog(
  ...
  try {
    ...
    const applicationTyped = application as { client_id?: string } | null;
    ...
    await logAuditEvent({
      ...
      record_id: serviceLogData?.id || "",
      new_values: (serviceLogData || {}) as Record<string, unknown>,
      application_id: input.application_id,
      client_id: clientId,
...
export async function updateServiceLog(
  ...
  try {
    ...
    const existingLogTyped = existingLog as { applications?: { client_id?: string }; cost_total?: number | null; application_id?: string } | null;
    const clientId = existingLogTyped?.applications?.client_id || ""

    // 수리비가 변경되고 한도 초과 가능성이 있는 경우 체크
    if (
      input.service_type === "repair" &&
      input.cost_total !== undefined &&
      input.cost_total !== null &&
      input.cost_total !== existingLogTyped?.cost_total &&
      input.cost_total > 0
    ) {
      const limitCheck = await checkRepairLimit(clientId, input.cost_total)
    ...
    await logAuditEvent({
      ...
      application_id: existingLogTyped?.application_id || undefined,
      client_id: clientId,
