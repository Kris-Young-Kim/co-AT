  ...
  try {
    ...
    try {
      ...
      await adminSupabase.from('notifications').insert({
        type: 'schedule',
        title: '새 상담 예약 신청',
        body: `${APPOINTMENT_SERVICE_LABELS[input.service_type] ?? input.service_type} 예약 신청이 접수되었습니다`,
        is_global: true,
      })
    } catch { /* non-critical */ }
    ...
    try {
      ...
      if (r.appointment_slots && r.slot_id && input.assignedStaffId) {
        ...
        const { data: sched } = await adminSupabase
          .from('schedules')
          .insert({
            title: `상담 예약 — ${APPOINTMENT_SERVICE_LABELS[r.service_type] ?? r.service_type}`,
      ...
      try {
        ...
        await adminSupabase.from('notifications').insert({
          type: 'schedule',
          title: '예약이 확정되었습니다',
          body: `${APPOINTMENT_SERVICE_LABELS[r.service_type] ?? r.service_type} 예약이 확정되었습니다. 담당자가 연락드릴 예정입니다.`,
      ...
      if (req) {
        ...
        try {
          ...
          await adminSupabase.from('notifications').insert({
            ...
            body: `${APPOINTMENT_SERVICE_LABELS[r.service_type] ?? r.service_type} 예약 신청이 반려되었습니다.${staffNote ? ` 사유: ${staffNote}` : ''}`,
            clerk_user_id: r.portal_user_id,
          })
        } catch { /* non-critical */ }
      }
