...
import { CATEGORY_LABELS, SUB_CATEGORY_LABELS, SCHEDULE_TYPE_LABELS, SERVICE_RECORD_TYPE_LABELS } from "@/lib/constants/mappings"
  ...
  return withStaffPermission(async () => {
    try {
      ...
      applications?.forEach((app: { id: string; category: string | null; sub_category: string | null; status: string | null; created_at: string | null }) => {
        const categoryName = CATEGORY_LABELS[app.category || ""] || app.category || "기타"
        const subCategoryName = SUB_CATEGORY_LABELS[app.sub_category || ""] || app.sub_category || ""
        ...
        history.push({
          ...
          schedules?.forEach((schedule: { id: string; schedule_type: string | null; scheduled_date: string | null; scheduled_time: string | null; status: string | null; created_at: string | null }) => {
            const typeName = SCHEDULE_TYPE_LABELS[schedule.schedule_type || ""] || schedule.schedule_type || "일정"
      ...
      serviceLogs?.forEach((log: { id: string; service_type: string | null; service_area: string | null; service_date: string | null; created_at: string | null }) => {
        const typeName = SERVICE_RECORD_TYPE_LABELS[log.service_type || ""] || log.service_type || "서비스"
