  ...
  try {
    ...
    if (error) {
      ...
      return {
        success: false,
        error:
          "평가 생성에 실패했습니다: " + (error.message || "알 수 없는 오류"),
      };
    }
...
export async function updateDomainAssessment(
  ...
  try {
    ...
    if (updates.evaluation_date)
      updateData.evaluation_date = updates.evaluation_date;
    if (updates.evaluation_data !== undefined)
      updateData.evaluation_data = updates.evaluation_data;
    if (updates.measurements !== undefined)
      updateData.measurements = updates.measurements;
    if (updates.evaluator_opinion !== undefined)
      updateData.evaluator_opinion = updates.evaluator_opinion;
    if (updates.recommended_device !== undefined)
      updateData.recommended_device = updates.recommended_device;
    if (updates.future_plan !== undefined)
      updateData.future_plan = updates.future_plan;
